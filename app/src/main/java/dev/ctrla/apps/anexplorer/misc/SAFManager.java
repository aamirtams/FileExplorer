...
import static dev.ctrla.apps.anexplorer.misc.IntentUtils.ACTION_OPEN_DOCUMENT_TREE;
import static dev.ctrla.apps.anexplorer.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return dev.ctrla.apps.anexplorer.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return dev.ctrla.apps.anexplorer.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return dev.ctrla.apps.anexplorer.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return dev.ctrla.apps.anexplorer.model.DocumentsContract.isTreeUri(uri);
    }
